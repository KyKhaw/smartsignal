...
class LambdaMARTRanker:
    ...
    def predict(self, test_panel: pd.DataFrame) -> pd.Series:
        ...
        Returns
        -------
        scores : pd.Series indexed like test_panel.
        """
        if self._ranker is None:
            raise RuntimeError("Call fit() before predict().")

        test_sorted = test_panel.sort_index().sort_values("ticker", kind="stable")
        X_te = self._scaler.transform(
            test_sorted[self.feature_cols].values.astype(np.float32)
        )[:, self._sel_idx]
        scores = self._ranker.predict(X_te)
