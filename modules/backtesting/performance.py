...
class PerformanceAnalyser:
    ...
    def annual_returns(self) -> pd.Series:
        ...
        return ((1 + self.returns).resample("YE").prod() - 1).rename("annual_return")
...
def information_coefficient(
    ...
    sc = scores.reindex(common).dropna()
    fr = fwd_ret.reindex(common).dropna()
    idx = sc.index.intersection(fr.index)
    sc, fr = sc.reindex(idx), fr.reindex(idx)

    if not by_date:
        ic = float(sc.corr(fr, method="spearman"))
        return {"ic_mean": ic, "ic_std": np.nan, "icir": np.nan, "ic_positive_pct": np.nan}

    daily_ic = (
        pd.DataFrame({"score": sc, "fwd_ret": fr})
          .groupby(level=0)
