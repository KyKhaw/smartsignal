...
def compare_strategies(
    ...
    for row in pct_rows:
        if row in df.index:
            df.loc[row] = df.loc[row].map(lambda x: f"{x:.2%}" if pd.notna(x) else "NaN")

    float_rows = ["sharpe", "sortino", "calmar", "profit_factor",
                  "skewness", "kurtosis", "recovery_factor"]
    for row in float_rows:
        if row in df.index:
            df.loc[row] = df.loc[row].map(lambda x: f"{x:.3f}" if pd.notna(x) else "NaN")
