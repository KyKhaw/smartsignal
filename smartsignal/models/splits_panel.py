...
class PanelWalkForwardSplitter:
    ...
    def __init__(
        ...
        # None → auto-detect from actual panel in split()
    ...
    def split(
        ...
        # Auto-detect min_tickers threshold from actual universe size
        # Use 50% of median daily count, floored at 5
        if self.min_tickers_per_date is None:
            daily_counts = panel.groupby(level=0).size()
            threshold = max(5, int(daily_counts.median() * 0.5))
        else:
            threshold = self.min_tickers_per_date
        ...
        for train_dates, test_dates in self._splitter.split(dates):
            ...
            # Breadth check using resolved threshold
            if not self._breadth_ok(train_panel, threshold):
                continue
            if not self._breadth_ok(test_panel, threshold):
                continue

            yield train_panel, test_panel

    def _breadth_ok(self, panel: pd.DataFrame, threshold: int) -> bool:
        """Return True if median daily ticker count meets the minimum."""
        daily_counts = panel.groupby(level=0).size()
        return daily_counts.median() >= threshold
