...
def build_daily_positions(
    ...
    # Auto-scale min_universe_size: never require more tickers than actually exist
    actual_median = scored.groupby(level=0).size().median()
    effective_min = min(min_universe_size, max(n_long + n_short, int(actual_median * 0.5)))
    ...
    for dt in rebalance_dates:
        ...
        # Universe size guard (uses auto-scaled effective minimum)
        if len(day_data) < effective_min:
            if verbose:
        ...
        # Clamp leg sizes so they never exceed available tickers
        avail   = len(tickers)
        n_l     = min(n_long,  avail // 2)
        n_s     = min(n_short, avail // 2)
        if n_l == 0 or n_s == 0:
            raw_positions[dt] = {t: 0 for t in tickers}
            continue

        new_pos: Dict[str, int] = {t: 0 for t in tickers}

        # Long: top n_l
        for t in tickers[:n_l]:
